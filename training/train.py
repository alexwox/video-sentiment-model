...
SM_CHANNEL_TESTING = os.environ.get('SM_CHANNEL_TEST', "/opt/ml/input/data/test")
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        device = torch.device('mps')  # Apple's Metal Performance Shaders backend (Neural Engine)
    else:
        device = torch.device('cpu')

    print(f"Using device: {device}")
    
    #Traick inital GPU memory if available
    if torch.cuda.is_available():
    ...
    for epoch in tqdm(range(args.epochs), desc="Epochs"):
        ...
        if val_loss["total"]< best_val_loss:
            ...
            torch.save(model.state.dict(), os.path.join(args.model_dir, "model.pth"))
            
    # After completed training, evaluate on test
